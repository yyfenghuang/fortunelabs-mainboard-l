...
esp_err_t i2c_bus_init(i2c_bus_t *bus, const i2c_bus_config_t *cfg)
{
    ...
    ESP_LOGI(TAG, "Bus init OK — port=%d  SDA=%d  SCL=%d  clk=%luHz",
             cfg->port, cfg->sda_pin, cfg->scl_pin,
             (unsigned long)cfg->clk_hz);
...
esp_err_t i2c_bus_add_device(i2c_bus_t *bus, uint8_t addr, uint32_t scl_hz, const char *label, i2c_master_dev_handle_t *out)
{
    ...
    {
        ESP_LOGE(TAG, "Device registry full (%d/%d)",
                 bus->device_count, I2C_BUS_MAX_DEVICES);
        return ESP_ERR_NO_MEM;
    }
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to add device 0x%02X (%s): %s",
                 addr, label ? label : "?", esp_err_to_name(ret));
        return ret;
    }
    ...
    ESP_LOGI(TAG, "Device aded: 0x%02X (%s), - %lu Hz [%d/%d slots]",
             addr, label ? label : "?", (unsigned long)scl_hz, bus->device_count, I2C_BUS_MAX_DEVICES);
...
esp_err_t i2c_bus_write(i2c_bus_t *bus, i2c_master_dev_handle_t dev, const uint8_t *data, size_t len)
{
    ...
    {
        bus->error_count++;
        ESP_LOGW(TAG, "Write failed %s (errors: %lu)", esp_err_to_name(ret), (unsigned long)bus->error_count);
    }
    return ret;
}
...
esp_err_t i2c_bus_read(i2c_bus_t *bus, i2c_master_dev_handle_t dev, uint8_t *buf, size_t len)
{
    ...
    {
        bus->error_count++;
        ESP_LOGW(TAG, "Read failed: %s (errors: %lu)",
                 esp_err_to_name(ret), (unsigned long)bus->error_count);
    }
    return ret;
}
...
esp_err_t i2c_bus_write_read(i2c_bus_t *bus, i2c_master_dev_handle_t dev, const uint8_t *wr_data, size_t wr_len, uint8_t *rd_buf, size_t rd_len)
{
    ...
    esp_err_t ret = i2c_master_transmit_receive(dev, wr_data, wr_len, rd_buf, rd_len, I2C_BUS_TIMEOUT_MS);
    xSemaphoreGive(bus->mutex);

    if (ret != ESP_OK)
    {
        bus->error_count++;
        ESP_LOGW(TAG, "Write-read failed: %s (errors: %lu)", esp_err_to_name(ret), (unsigned long)bus->error_count);
    }
    return ret;
}
...
{
    return bus ? bus->error_count : 0;
}

uint8_t i2c_bus_get_device_count(const i2c_bus_t *bus)
{
    return bus ? bus->device_count : 0;
}
